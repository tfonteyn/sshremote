    ...
    public void connect(@NonNull final String host,
        ...
        try {
            ...
            */
            ...
            boolean check = false;
            final int authMethod = buf[1] & 0xff;
            ...
            switch (authMethod) {
                ...
                case 2: { // USERNAME/PASSWORD
                    if (user == null || passwd == null) {
                        failureMsg = "SOCKS5 username/password authentication requested"
                                     + " by the server but no username/password configured";
                        break;
                    }
                    /*
                ...
                default: {
                    failureMsg = String.format(
                            Locale.ROOT,
                            "Unsupported SOCKS5 authentication method requested"
                            + " by the server: %02X", authMethod);
                    break;
                }
            }

            if (!check) {
                throw new SshProxyException(failureMsg);
            }
